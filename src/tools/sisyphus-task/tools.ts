...
const CATEGORY_EXAMPLES = Object.keys(DEFAULT_CATEGORIES)
  .map((k) => `'${k}'`)
  .join(", ");
...
export function createSisyphusTask(
  options: SisyphusTaskToolOptions,
): ToolDefinition {
  const { manager, client, directory, userCategories, gitMasterConfig } =
    options;
  ...
  return tool({
    ...
    args: {
      ...
      category: tool.schema
        ...
        .optional()
        .describe(
          `Category name (e.g., ${CATEGORY_EXAMPLES}). Mutually exclusive with subagent_type.`,
        ),
      subagent_type: tool.schema
        .string()
        .optional()
        .describe(
          "Agent name directly (e.g., 'oracle', 'explore'). Mutually exclusive with category.",
        ),
      run_in_background: tool.schema
        .boolean()
        .describe(
          "Run in background. MUST be explicitly set. Use false for task delegation, true only for parallel exploration.",
        ),
      resume: tool.schema
        .string()
        .optional()
        .describe(
          "Session ID to resume - continues previous agent session with full context",
        ),
      skills: tool.schema
        .array(tool.schema.string())
        .describe(
          "Array of skill names to prepend to the prompt. Use [] if no skills needed.",
        ),
    },
    async execute(args: SisyphusTaskArgs, toolContext) {
      ...
      if (args.skills.length > 0) {
        ...
        if (notFound.length > 0) {
          const available = createBuiltinSkills()
            .map((s) => s.name)
            .join(", ");
...
Use \`background_output\` with task_id="${task.id}" to check progress.`;
        ...
        const messages = (
          (messagesResult as { data?: unknown }).data ?? messagesResult
        ) as Array<{
...
${textContent || "(No text output)"}`;
      ...
      if (args.category) {
        ...
        if (!resolved) {
          return `❌ Unknown category: "${args.category}". Available: ${Object.keys(
        ...
        try {
          ...
          if (!callableNames.includes(agentToUse)) {
            const isPrimaryAgent = agents.some(
              (a) => a.name === agentToUse && a.mode === "primary",
            );
            if (isPrimaryAgent) {
...
System notifies on completion. Use \`background_output\` with task_id="${task.id}" to check.`;
      ...
      try {
        ...
        while (Date.now() - pollStart < MAX_POLL_TIME_MS) {
          ...
          if (currentMsgCount > 0 && currentMsgCount === lastMsgCount) {
            ...
            if (stablePolls >= STABILITY_POLLS_REQUIRED) {
              log("[sisyphus_task] Poll complete - messages stable", {
                sessionID,
                pollCount,
                currentMsgCount,
              });
        ...
        if (Date.now() - pollStart >= MAX_POLL_TIME_MS) {
          log("[sisyphus_task] Poll timeout reached", {
            sessionID,
            pollCount,
            lastMsgCount,
            stablePolls,
